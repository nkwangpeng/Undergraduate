...
alloc_proc(void) {
    /*
    alloc_proc函数（位于kern/process/proc.c中）负责分配并返回一个新的struct proc_struct结构，用于存储新建立
    的内核线程的管理信息。ucore需要对这个结构进行最基本的初始化。
    */
     ...
     */
        ...
        proc->cr3 = boot_cr3;    //pmm.c    页目录表基址设为内核页目录表基址
        proc->pid = -1;      //表示未分配
        proc->runs = 0; // 对其他成员变量清零处理
        proc->kstack = 0;  ////内核栈，记录了分配给该线程的内核栈的位置
        proc->need_resched = 0;
        proc->parent = NULL;
        proc->mm = NULL;
