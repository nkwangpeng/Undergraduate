...
<body>
    ...
    <div class="page-heading-85912" style="background-image: url('img/hero_bg_3.jpg')">
      <div class="container">
        <div class="row align-items-end">
          <div class="col-lg-7">
            <h1 class="mb-0 heading text-white">全国疫情图</h1>
  ...
  <script type="text/javascript" >
    ...
    option1={
      ...
      visualMap: {
        ...
        bottom:'40px',
        pieces:[
          {min:1000,label:'>1000',color:'#372a28'},
          {min:501,max:1000,label:'501-1000',color:'#4e160f'},
          {min:101,max:500,label:'101-500',color:'#974236'},
          {min:11,max:100,label:'11-100',color:'#ee7263'},
          {min:1,max:10,label:'1-10',color:'#f5bba7'},
          {min:0,max:0,lable:'0',color:'#f7f7f7'},
        ],
      ...
      "tooltip": {
            ...
            for(var i=0;i<mapdatatool.length;i++){
                ...
                res+='</br>' +'现存确诊数: '+mapdatatool[i].value;
                break;
        }
    ...
    function getnowData(){
      ...
      $.getJSON("index.php?r=chinamap/data", function (data) {
      var json = JSON.parse(JSON.stringify(data).replace(/province/g,"name"));
      var json1 = JSON.parse(JSON.stringify(json).replace(/diagnosis/g,"value"));
      dataSource=json1;
      var mapData=[];
       for (var i = 0; i < data.length; i++) {
        ...
        //if (geoCoord) {
            mapData.push({
                ...
                "value": geo.concat("diagnosis",data[i]["diagnosis"],"totaldiagnosis",data[i]["totaldiagnosis"])
            });
        //}
       }
      mapdatatool=mapData;
    console.log(dataSource);
      ...
      $.getJSON("index.php?r=chinamap/data", function (data) {
      var json = JSON.parse(JSON.stringify(data).replace(/province/g,"name"));
      var json1 = JSON.parse(JSON.stringify(json).replace(/totaldiagnosis/g,"value"));
      dataSource=json1;
    console.log(dataSource);
    ...
    function freshbackgroud(currentConfirmedCount, confirmedCount) {
      ...
      /*实时数据背景*/
      $('#realTimeData').css({ 'background-color': 'rgba(130, 210, 10)' });
    ...
    var tipContent = ""
      ...
      + "4、2020年3月19日起每日数据不再更新，请参考卫健委官方数据。<br>";
    var tip_index = 0;
  ...
  <script>
    ...
    $.ajax({
    type : "post",
    url : "index.php?r=site/update-current",
    dateType : "json",
