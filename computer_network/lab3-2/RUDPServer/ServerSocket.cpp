...
void listen_ack(ServerSocket* skt) {
	...
	while (true) {
		...
		//recvhd->seq > lar && recvhd->seq <= lfs &&
		if (recvhd->seq > lar && recvhd->seq <= lfs && checkSum==0) {
			if (recvhd->isAck) {
				window_ack_mask[recvhd->seq - (lar + 1)] = true;
				CString file(recvhd->fileName);//显示日志记录
				CString out;
				out.Format(L":(ACK) %d  \n", recvhd->seq);
				out = L"\r\n" + out;
				out = out + file;
				...
				window_sent_time[recvhd->seq - (lar + 1)] = TMIN;
			}
		}
...
void ServerSocket::Reply110(CString s, int clientPort, int serverPort, ServerSocket& skt) {
	...
	std::thread recv_thread(listen_ack, &skt);
