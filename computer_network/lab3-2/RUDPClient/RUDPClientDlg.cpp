...
//用于接收收到的文件分片
bool CRUDPClientDlg::receiveFile(ClientSocket& skt, int clientPort,bool * window_recv_mask) {
	...
	int checkSum = ChecksumCompute((unsigned short*)m_szBuffer, nRead);
	...
	default:
		...
		else if(checkSum == 0){//收到文件报文的话
			header* recvhd = (header*)(m_szBuffer);
			memcpy(umap[recvhd->seq], m_szBuffer, 50000);
			string str(recvhd->fileName);
			if (fileSeq.find(str) == fileSeq.end()) {//如果文件名不在map容器里，说明是新的文件，把它插入容器中
				fileSeq[str] = int(-1);
			}
			bool done = skt.processFile(skt, m_szBuffer, clientPort,window_recv_mask);//处理报文
			return done;
		}
	}
	return false;
...
//处理文件分片
bool ClientSocket::processFile(ClientSocket& skt, u_char* raw, int clientPort, bool* window_recv_mask) {
	CRUDPClientDlg* pdlg = (CRUDPClientDlg*)AfxGetApp()->GetMainWnd();
	...
	if (lfr >= (recvhd->fileSize)/MAX_PACKET) { //当报文中数据段的长度小于报文最大长度的时候，说明接收完毕，关闭socket
...
UINT Downloader(PVOID hWnd) // 下载线程的控制函数
{
	...
	while (1) {
		skt.Listen();
		bool done = pdlg->receiveFile(skt, port,window_recv_mask);
		if (done)
			break;
	}
	return 0;
